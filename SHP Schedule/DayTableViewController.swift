 ...
 */
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "DayTableCell", for: indexPath) as! DayTableViewCell
        ...
        cell.periodLabel?.textColor = UIColor.black
        ...
        if dayForView?.toDateString() == Date().toDateString()
        {
            ...
            var startComponents = calendar.dateComponents([Calendar.Component.hour, Calendar.Component.minute], from: (startTimeMil?.toDate(withFormat: "hh:mm")) ?? Date())
            ...
            let endComponents = calendar.dateComponents([Calendar.Component.hour, Calendar.Component.minute], from: (endTimeMil?.toDate(withFormat: "hh:mm")) ?? Date())
            ...
            {
                cell.periodLabel?.textColor = UIColor.schoolColor
                cell.timeLabel?.textColor = UIColor.schoolColor
            }
        }
        /*
        let detailLabelWidth:CGFloat = 100
        
        if var textLabelFrame = cell.textLabel?.frame {
            if textLabelFrame.width + detailLabelWidth > cell.frame.width {
                textLabelFrame.size.width -= detailLabelWidth
